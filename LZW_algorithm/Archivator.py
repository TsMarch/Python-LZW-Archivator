...
def archiver(text_encode: List[str]) -> str:
    ...
    сжатия строки. В данном случае парой ключ - значение является символ и его номер unicode. Словарь создается на
    основе входящего текста. Если падает ошибка ValueError, значит словарь не нашел символ в таблице unicode.
    Будет выведено сообщение с проблемным символом.
    """
